...
contract PancakeStableSwap is Ownable, ReentrancyGuard {
    ...
    function checkLpBalance(address user) external view returns (uint256) {
        return token.balanceOf(user);
    }

    function checkInnerBalance(uint256 i) external view returns (uint256) {
        uint256[N_COINS] memory old_balances = balances;
        return old_balances[i];
    }
    ...
    function add_liquidity(uint256[N_COINS] memory amounts, uint256 min_mint_amount) external nonReentrant {
        ...
        if (token_supply > 0) {
            ...
            emit Kill();
            emit Info(new_balances[0]);
            emit Info(new_balances[1]);
            emit Kill();
        }
    ...
    function exchange(
        ...
        uint256[N_COINS] memory old_balances = balances;
        emit Info(old_balances[0]);
        emit Info(old_balances[1]);
